...
провайдеров, извлекает поддерживаемые серверы, удаляет дубликаты и выдаёт их как
одну подписку. В дашборде можно добавлять источники без перезапуска, создавать
отдельные профили и менять порядок серверов перетаскиванием.
...
порядок серверов. Дубликаты определяются без учёта названия после `#`.
