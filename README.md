...
- Параллельное автообновление всех источников перед выдачей профиля.
...
| `SUBSCRIPTION_REFRESH_SECONDS` | `900` | Параметр совместимости; автообновление теперь выполняется перед каждой выдачей |
...
Все admin-маршруты объединены под `/api` и защищены общей зависимостью авторизации. Публичные `/healthz`, `/readyz` и `/s/{token}` изолированы в отдельном роутере. FastAPI также раздаёт production-сборку SPA под `/admin/`.
...
После запуска проверьте `GET /healthz`: это liveness-проверка процесса без обращения к базе. `GET /readyz` дополнительно выполняет запрос к базе и возвращает `503`, если соединение недоступно. Для частых проверок платформы используйте `/healthz`, а `/readyz` опрашивайте реже отдельным мониторингом.
